    ...
    static test() {
        ...
        describe(this.describe, () => {
            ...
            test("Rota GET com um parametro", async () => {
                const resposta = await request(app).get(this.rootPath + this.paramPath + this.param)
            ...
            test("Rota PUT", async () => {
                const respostaPut = await request(app).put(this.rootPath + this.paramPath + this.param)
                    .set('user', 'admin')
                    .send(this.putAtt)
                expect(respostaPut.status).toBe(200)
                const respostaGet = await request(app).get(this.rootPath + this.paramPath + this.paramAtt)
                this.setDados(respostaGet.body.dados)
                expect(this.expect).toBe(this.paramAtt)
            })
            test("Rota DELETE", async () => {
                const respostaDelete = await request(app).delete(this.rootPath + this.paramPath + this.paramAtt)
                    .set('user', 'admin')
